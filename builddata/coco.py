"""split_name,
    Convert COCO data into TFRecords
"""
...
def process_metadata(split_name, caption_data, image_dir):
    ...
    for img in caption_data:
        count += 1
        label = img["imgid"]
        filename = os.path.join(image_dir, img["filepath"], img["filename"])
        # print(filename)
...
def run():
    ...
    val_caption_data = [img for img in caption_data['images'] if img['split'] == 'restval']
    ...
    val_dataset = process_metadata("restval", val_caption_data, FLAGS.image_dir)
